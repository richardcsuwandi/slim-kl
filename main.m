...
Delta = 0; % Quantization resolution; 0 for no quantization
    ...
    for j = 1:N
        ...
        for k = 1:options.local_max_iter
            ...
            for i = 1:S
                ...
                [zeta_j_i_hat, nv] = mosek_optimize(y_train_j, L_j_i, C_k_j, C_minus, ...
...
% Save info
save(['./fig/',file_name,'_rho',num2str(rho_init), '_maxiter',int2str(options.max_iter), '_N', int2str(options.N), '_Delta', num2str(options.Delta), '.mat'], ...
